...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    if (activePeriod != null) {
      if (criteria.getActivePeriod().getStartingDay() != null) {
        query
            .where(Alias.$(special.getActivePeriod().getStartingDay()).eq(criteria.getActivePeriod().getStartingDay()));
      }

      if (criteria.getActivePeriod().getStartingHour() != null) {
        query.where(Alias.$(special.getActivePeriod().getStartingHour()).eq(
            criteria.getActivePeriod().getStartingHour()));
      }

      if (criteria.getActivePeriod().getEndingHour() != null) {
        query.where(Alias.$(special.getActivePeriod().getEndingHour()).eq(criteria.getActivePeriod().getEndingHour()));
      }

      if (criteria.getActivePeriod().getEndingDay() != null) {
        query.where(Alias.$(special.getActivePeriod().getEndingDay()).eq(criteria.getActivePeriod().getEndingDay()));
      }
